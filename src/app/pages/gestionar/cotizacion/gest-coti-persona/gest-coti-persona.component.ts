...
export class GestCotiPersonaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.terminoBusquedaCotizacion.valueChanges.subscribe(() => this.buscarCotizaciones());
  }
  ...
  columnasTablaServiciosCotizados: string[] = ['accion', 'codigo', 'tipo', 'nombre', 'cantidad', 'precioLista', 'diferencia', 'precioVenta', 'descuentoPorcentaje', 'totalUnitario'];
  ...
  setPacienteSeleccionado(paciente: any) {
    ...
    this.myFormCotizacion.patchValue({
      nomCliente: `${paciente.apePatCliente} ${paciente.apeMatCliente} ${paciente.nombreCliente}`.trim(),
  ...
  setSolicitanteSeleccionado(solicitante: any) {
    ...
    this.myFormCotizacion.patchValue({
      ...
      nomSolicitante: `${solicitante.apePatRefMedico} ${solicitante.apeMatRefMedico} ${solicitante.nombreRefMedico}`.trim(),
  ...
  ultimosServicios(cantidad: number): void {
    this._servicioService.getLastServicio(cantidad).subscribe({
      next: (res: IServicio[]) => { this.servicios = res; },
      error: (err: any) => { this.servicios = []; },
    });
  }

  listarServiciosFrecuentes() {
    this._servicioService.getAllFavoritesServicios().subscribe({
      next: (res: IServicio[]) => { this.serviciosFrecuentes = res; },
      error: (err: any) => { this.serviciosFrecuentes = []; },
    });
  }
  ...
  buscarServicio() {
    ...
    this.timeoutBusqueda = setTimeout(() => {
      ...
      if (termino.length >= 3) {
        this._servicioService.getServicio(termino).subscribe((res: IServicio[]) => {
          this.servicios = res;
        });
      } else if (termino.length > 0) {
        this.servicios = [];
  ...
  seleccionarServicio(servicio: IServicio) {
    ...
    if (existe) {
      Swal.fire({ icon: 'warning', title: 'Servicio ya agregado', text: 'Este servicio ya está en la cotización.' });
      return;
    }
    ...
    const servicioForm = this._fb.group({
      ...
      precioLista: [servicio.precioServicio, [Validators.required, Validators.min(0)]],
      diferencia: [0, [Validators.min(0)]],
      precioVenta: [servicio.precioServicio, [Validators.required, Validators.min(0), Validators.pattern(/^[0-9]+(\.[0-9]{1,2})?$/)]],
      descuentoPorcentaje: [0, [Validators.min(0), Validators.max(100)]],
      totalUnitario: [servicio.precioServicio, [Validators.required, Validators.min(0)]],
    });
  ...
  calcularTotalUnitario(index: number) {
    ...
    let totalUnitario = cantidad * Math.round((precioVenta * ((100 - descuentoPorcentual) / 100)) * 100) / 100;
  ...
  removerServicio(index: number) {
    ...
    if (this.tienePagos) {
      console.warn('No se puede eliminar un servicio porque ya tiene pagos asociados');
      return;
    }
  ...
  @ViewChild('inputPrecioGlobal') inputPrecioGlobal!: ElementRef<HTMLInputElement>;
  @ViewChild('inputPorcentajeGlobal') inputPorcentajeGlobal!: ElementRef<HTMLInputElement>;
  ...
  calcularTotalGeneral() {
    ...
    this.serviciosCotizacion.controls.forEach(control => {
      ...
      const totalUnitario = parseFloat(control.get('totalUnitario')?.value) || 0; // Obtener el total unitario
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this._cotizacionService.generarNuevaVersion(nuevaCotizacion).subscribe({
          ...
          error: (error) => {
            const mensaje = error?.error?.msg || 'Error inesperado al generar la versión.';
            this.mostrarAlertaError(mensaje);
          }
        });
  ...
  validarformulario(): boolean{
    ...
    const nomClienteValor = nomClienteControl ? nomClienteControl.getRawValue() : '';
  ...
  cargarCotizacion(cotizacion: any) {
    ...
    this.versionesDisponibles = cotizacion.historial.map((h: any) => h.version as number).sort((a: number, b: number) => a - b);
  ...
  cargarVersion(version: number) {
    ...
    const historialVersion = this.cotizacionCargada.historial.find((h: any) => h.version === version);
  ...
  generarPDF(preview: boolean) {
    ...
    const pdfSrc = this._pdfService.generarPDFCotizacion(this.cotizacionParaImprimir, preview);
